...
```sql
SOURCE database/create_schema.sql;
SOURCE database/initialize_data.sql;
```
...
- If MySQL uses a different port, schema name, username, or password, update them in `src/main/resources/db.properties`.
- If Tomcat is already running an older deployment, stop Tomcat, replace the WAR in `webapps`, and restart it.
- If MySQL scripts are run from another working directory, use absolute paths that match your local machine.
...
- Connection and configuration failures throw clear exceptions from `src/main/java/com/bank/config/DatabaseConfig.java` and `src/main/java/com/bank/util/ConnectionFactory.java`.
